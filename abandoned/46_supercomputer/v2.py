...
class RedBlackTree:
  ...
  def count_in_range(self, f: int, t: int):
    count = 0
    queue = deque([self.get_root()])
    while len(queue) > 0:
      curr_node = queue.popleft()
      if curr_node.data < f:
        if curr_node.right != self.NULL_NODE:
          queue.append(curr_node.right)
      elif curr_node.data > t:
        if curr_node.left != self.NULL_NODE:
          queue.append(curr_node.left)
      else:
        count += 1
        if curr_node.left != self.NULL_NODE: queue.append(curr_node.left)
        if curr_node.right != self.NULL_NODE: queue.append(curr_node.right)
    return count
  ...
  for _ in range(K):
    ...
    if tokens[0] == "F":
      ...
      if memory[index]:
        ...
        tree.delete_node(index)
    
    else:
      f, t = int(tokens[1]), int(tokens[2])
      count = tree.count_in_range(f, t)
      print(count)
